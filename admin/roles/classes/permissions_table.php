...
class core_role_permissions_table extends core_role_capability_table_base {
    ...
    protected function add_row_cells($capability) {
        ...
        foreach ($roles as $id => $name) {
            if (isset($needed[$id])) {
                $spancontent = $roles[$id].' ';
                if (isset($overridableroles[$id]) and ($allowoverrides or ($allowsafeoverrides and is_safe_capability($capability)))) {
                    $preventurl = new moodle_url($PAGE->url, array('contextid' => $contextid, 'roleid' => $id,
                                                 'capability' => $capability->name, 'prevent' => 1));
                    $spancontent .= $OUTPUT->action_icon($preventurl, new pix_icon('t/delete', get_string('prevent', 'core_role')), null,
                            array('data-role-id' => $id, 'class' => 'preventlink', 'data-action' => 'prevent'));
                }
                $neededroles[$id] = html_writer::span($spancontent, 'allowed', array ('style' => 'display:inline-block;'));;
            }
        }
        ...
        foreach ($roles as $id => $name) {
            if (isset($forbidden[$id])  and ($allowoverrides or ($allowsafeoverrides and is_safe_capability($capability)))) {
                ...
                if (isset($overridableroles[$id]) and prohibit_is_removable($id, $context, $capability->name)) {
                    $unprohibiturl = new moodle_url($PAGE->url, array('contextid' => $contextid, 'roleid' => $id,
                                    'capability' => $capability->name, 'unprohibit' => 1));
                    $spancontent .= $OUTPUT->action_icon($unprohibiturl, new pix_icon('t/delete', get_string('delete')),null,
                            array('data-role-id' => $id, 'class' => 'unprohibitlink', 'data-action' => 'unprohibit'));
                }
                $forbiddenroles[$id] = html_writer::span($spancontent, 'forbidden', array ('style' => 'display:inline-block;'));
            }
        }
