...
/*cis in futeure versions this might change*/
$filearea = optional_param('filearea', 'draft', PARAM_ALPHA);
...
foreach ($files as $file) {
    ...
    if ($existingfile) {
        ...
        //test-cis
        $data= new stdClass();
        $options=array("subdirs"=>1,"maxbyte"=>104857600,"maxfiles"=>-1,"accepted_types"=>"*");
        $draftid=file_prepare_standard_filemanager($data, 'files', $options,$context,'user','private',0);
        $results[] = $file_record;
        $image_url = new stdClass;
        $image_url->url=$CFG->wwwroot."/draftfile.php/".$results[0]->contextid."/user/draft/".$draftid->files_filemanager.$filepath.$results[0]->filename;
        $image_url->draftitemid=$draftid->files_filemanager;
        $image_url->path=$filepath;
        $image_url->base=$CFG->wwwroot."/draftfile.php/".$results[0]->contextid."/user/draft/";
        $results=$image_url;
    }
}
